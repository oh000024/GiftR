...
var messages = [
  {"type":"bad", "msg":"Input data is not complete"},
  {"type":"good", "msg":"I just ordered pizza"},
  {"type":"info", "msg":"Today is Friday"}
];

//ev. preventDefault();
//ev.stopPropagagion() // do not move up the chain of objects
//ev.stopImmediagteProg=pagation(); // do no t other event listeners on the same object
...
const GAPP = {
	...
	createHtml4Gifts: function (person) {
		...
		document.querySelector(".title").textContent = "For " + person.name;
		let pname = document.querySelector("#tname.content-padded");
...
const MODALHANDLER = {
	...
	addPersonCancel: function () {
		...
		globalPersonId = 0;
		 ...
		 //doesn't work, review it again
//		let cancel = new CustomEvent('touchend');
//		let cbut = window.document.querySelector("a.icon.icon-close");
//		cbut.dispatchEvent(cancel);
	},
		...
		return function () {
			try {
				...
				if (idea.length == 0) {
					...
					//GAPP.addMessage({"type":"bad","message":"Incorrect input data"});
					return;
				}
